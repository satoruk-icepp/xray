...
void xray_analysis(){
  ...
  Double_t CFRPGap[4]={0,0.9,2.1,3.7};
  ...
  for(int iCh=0;iCh<nMPPC;iCh++){
	xrayac->GetEntry(iCh);
	if(DataQualPhi==true){
	  AllPhiPosGap[iCh]=PhiPos-PhiPosDesign;
	...
	if(DataQualZ==true&&std::abs(ZPos)<120){
	  ...
	  for(int i=0;i<4;i++){
		if(chline>=CFRPOrigin[i]&&chline<CFRPOrigin[i+1]){
		  ...
		  ZPosRealDesign=ZPosDesign+CFRPGap[i];
		  break;
		}
	  }
	  if(former==true){
