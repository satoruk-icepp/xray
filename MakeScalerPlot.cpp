...
void MakeScalerPlot(int runnum, int scan) {
...
void xray_ana(Int_t run, Int_t scan) {
  ...
  TTree *winner = new TTree("katta","katta");
  winner->Branch("nWD",&nWD);
  winner->Branch("indexMPPC",&indexMPPC);
  winner->Branch("mZ",&mZ);
  winner->Branch("mPhi",&mPhi);
  ...
  for (Int_t iScalerCh = 0; iScalerCh < kNchforXray; iScalerCh++) {
    if (valid[iScalerCh] && grScaler[iScalerCh]->GetN() >10){
      ...
      nWD = iScalerCh;
      indexMPPC = MPPCindex[iScalerCh];
      mZ = MPPCXYZ[iScalerCh][2];
      mPhi = MPPCPhi[iScalerCh];
      number_Yu += 1;
      winner->Fill();
      cout <<number_Yu<<" "<<iScalerCh<<" "<<MPPCindex[iScalerCh]<<" "<<MPPCPhi[iScalerCh]<<" "<<MPPCXYZ[iScalerCh][2]<<endl;
      iCh++;
      c1->cd(iCh);
